<style type='text/css'>
    ...
    #buttonbar a {
        float: left;
        background: url('<{$modurl}>/assets/images/left_both.gif') no-repeat left top;
        margin: 0;
        padding: 0 0 0 9px;
    ...
    #buttonbar a span {
        float: left;
        display: block;
        background: url('<{$modurl}>/assets/images/right_both.gif') no-repeat right top;
    ...
    <ul>
        <{foreach from=$adminObject key=itemnum item=menuitem}>
            <{if $itemnum==$current}>
                ...
                <li>
            <{/if}>
            <a href="<{$modurl}>/<{$menuitem.link}>"><span><{$menuitem.title}></span></a>
            </li>
        <{/foreach}>
    </ul>
</div>
